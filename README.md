...
1. **Emotion** - happy, sad, anxious, calm, stressed, content, overwhelmed, peaceful
2. **Trigger** - work stress, social conflict, health concerns, financial pressure, personal loss
3. **Activity** - exercise, journaling, meditation, rest, social connection, creative pursuits
4. **JournalEntry** - User's daily text-based emotional reflections
5. **Suggestion** - Breathing exercises, coping tips, affirmations, wellness recommendations
...
- **Jaseci** - Core framework for OSP, walkers, and byLLM
- **Jac Language** - DSL for graph and walker definitions
- **Python** - Backend runtime
- **OpenAI API / Ollama** - LLM provider (configurable)
...
- **SQLite / PostgreSQL** - Persistent storage for moods and journals
- **Jaseci Graph Store** - OSP persistence
...
│   ├── walkers.jac               # All walker implementations
│   ├── agents.jac                # AI agent logic (LLM calls)
│   ├── seed_data.jac             # Initial emotions, triggers, activities
│   ├── requirements.txt           # Python dependencies
│   ├── config.py                 # Configuration (LLM API keys, etc.)
│   └── seed_database.py          # Data initialization script
├── frontend/
│   ├── public/
│   │   ├── index.html
│   │   └── favicon.ico
